...
const FileUpload: React.FC<FileUploadProps> = ({
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      // This is necessary because `acceptedFileTypes` can contain glob-like patterns.
      const escapeRegex = (s: string) => {
        // Escape all regex special characters except '*' for glob replacement
        // Only escape the ones not typically found in mime types, or if they are, they aren't wildcards.
        return s.replace(/[-\\^$+?.()|[\]{}]/g, '\\$&');
      };

      let mimeTypePattern = acceptedFileTypes;
      if (acceptedFileTypes.endsWith('/*')) {
        // Convert glob 'image/*' to regex 'image/.*'
        mimeTypePattern = acceptedFileTypes.replace(/\/\*$/, '/.*');
      }
      // Escape any remaining regex special characters in the pattern
      mimeTypePattern = escapeRegex(mimeTypePattern);

      // Create a RegExp object to test the file type, anchoring it to match the whole string.
      // E.g., 'image/.*' becomes /^image\/.*$/
      const regex = new RegExp(`^${mimeTypePattern}$`);

      if (acceptedFileTypes !== '*/*' && !regex.test(file.type)) {
  ...
  return (
    <div className={className}>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {selectedFile && (
          <span className="text-sm text-[var(--color-text-primary)] truncate flex-1 min-w-[100px]" aria-live="polite">
